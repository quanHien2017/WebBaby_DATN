        ...
        <form role="form" action="{{ route(Request::segment(2) . '.update', $detail->id) }}" method="POST">
          ...
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="profile-username">@lang('Số thứ tự đơn hàng') #{{ $detail->id }}</h3>
              <p class="text-muted">{{ __('Thời gian tạo') }}: {{ $detail->created_at }}</p>
            </div>
            <div class="box-body">
              <div class="form-horizontal">
                <div class="form-group">
                  <label class="col-sm-3 text-right text-bold">@lang('Họ tên'):</label>
                ...
                <div class="form-group">
                  <label class="col-sm-3 text-right text-bold">@lang('Số điện thoại'):</label>
                  <label class="col-sm-9 col-xs-12">
                    {{ $detail->phone ?? '' }}
                  </label>
                </div>
