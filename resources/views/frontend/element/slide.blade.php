              ...
              @foreach($slide as $item)
                  @if($item->parent_id != '')
                   <div class="img has-hover x md-x lg-x y md-y lg-y" id="image_1395749462">
                      <a class="" href="{{ $item->url_link }}" target="_self" >
                         <div class="img-inner image-cover dark" style="padding-top:39.5%;">
                            <img width="820" height="492" src="{{ $item->image }}" data-src="{{ $item->image }}" class="lazy-load attachment-original size-original" alt="{{ $item->title }}" sizes="(max-width: 820px) 100vw, 820px" />						
                         </div>
                      </a>
                      <style scope="scope">
                         #image_1395749462 {
